...
bot.onText(/\/connect (.+)/, async (msg, match) => {
  ...
  if (!connectCode) {
    return sendError(
      chatId,
      "Please provide your connect code. e.g. `/connect CRYSTAL-a3f9b2`",
    );
  }
  ...
  try {
    ...
    if (!settings) {
      return sendError(
        chatId,
        "❌ Invalid connect code. Please check and try again.",
      );
    }
    ...
    if (settings.telegram?.connected) {
      return sendError(
        chatId,
        "✅ This store is already connected to Telegram.",
      );
    }
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendMessage(
    chatId,
    `👋 *Welcome to Crystal Store Keeper Bot!*\n\nI can give you real-time insights about your store.\n\nTo get started, connect your store by sending your connect code:\n\`/connect YOUR-CODE\`\n\nGet your connect code from *Settings → Integrations → Telegram* in your Crystal Store Keeper dashboard.`,
    { parse_mode: "Markdown" },
  );
});
...
bot.onText(/\/disconnect/, async (msg) => {
  ...
  try {
    const settings = await Setting.findOne({ "telegram.chatId": chatId });
