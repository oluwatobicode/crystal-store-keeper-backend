...
export const AI_TOOLS = [
  {
    ...
    description:
      "Get today's total sales, cash in register, pending payments count, and low stock alerts count",
    input_schema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Get total revenue, transaction count, average transaction value, and daily breakdown for a date range",
    input_schema: {
      type: "object",
      properties: {
        from: {
  ...
  },
  {
    ...
    description:
      "Get the best selling products by quantity sold in a date range",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Get all products that are at or below their reorder level and need restocking",
    input_schema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Get how much was collected via cash, POS, and bank transfer in a period, with percentages",
    input_schema: {
      type: "object",
      properties: {
        from: { type: "string", description: "Start date ISO string" },
  ...
  },
  {
    ...
    description:
      "Look up a customer by name or phone number — returns their balance, total spent, and status",
    input_schema: {
      type: "object",
      properties: {
        query: {
  ...
  },
  {
    ...
    description:
      "Get all sales with unpaid or partial payments and the total amount owed",
    input_schema: {
      type: "object",
      properties: {},
      required: [],
  ...
  try {
    switch (toolName) {
      ...
      case "get_sales_by_period": {
        ...
        const data = await getSalesByPeriod(businessId, from, to);
        return JSON.stringify(data);
      }

      case "get_top_selling_products": {
        const { from, to, limit = 5 } = toolInput;
        const data = await getTopSellingProducts(businessId, from, to, limit);
        return JSON.stringify(data);
      }

      case "get_low_stock_products": {
        const data = await getLowStockProducts(businessId);
        return JSON.stringify(data);
      }

      case "get_product_stock": {
        const { productName } = toolInput;
        const data = await getProductStock(businessId, productName);
        if (!data) {
      ...
      case "get_recent_transactions": {
        ...
        const data = await getRecentTransactions(businessId, limit);
        return JSON.stringify(data);
      }

      case "get_payment_method_breakdown": {
        const { from, to } = toolInput;
        const data = await getPaymentMethodBreakdown(businessId, from, to);
        return JSON.stringify(data);
      }

      case "get_customer_info": {
        const { query } = toolInput;
        const data = await getCustomerInfo(businessId, query);
        if (!data) {
      ...
      case "get_pending_payments": {
        const data = await getPendingPayments(businessId);
        return JSON.stringify(data);
      }
