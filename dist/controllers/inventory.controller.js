...
exports.getInventoryMovements = exports.adjustStock = exports.receiveStock = void 0;
...
const receiveStock = async (req, res, next) => {
    try {
        ...
        const product = await Product_1.default.findOne({ _id: productId, businessId });
        if (!product) {
        ...
        const movement = await StockMovement_1.default.create({
        ...
        await (0, auditLog_1.logAudit)(req.user._id, `${req.user.fullname}`, "RECEIVE_STOCK", `Received ${quantity} units of ${product.name} (${stockBefore} → ${stockAfter})`, "inventory", businessId);
...
const adjustStock = async (req, res, next) => {
    try {
        ...
        await (0, auditLog_1.logAudit)(req.user._id, `${req.user.fullname}`, "ADJUST_STOCK", `Adjusted ${product.name}: ${quantityChange > 0 ? "+" : ""}${quantityChange} units (${adjustmentType}) — ${reason}`, "inventory", businessId);
...
const getInventoryMovements = async (req, res, next) => {
    try {
        ...
        const movements = await StockMovement_1.default.find(filter)
            .sort({
            createdAt: -1,
        })
            .populate("productId");
